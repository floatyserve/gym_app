...
@Transactional
public class MembershipServiceJpa implements MembershipService {
    private final MembershipRepository membershipRepository;

    @Override
    public Membership findById(Long id) {
        return membershipRepository.findById(id)
                .orElseThrow(() -> new ReferenceNotFoundException("Membership not found with id: " + id));
    }
    ...
    public Membership create(Customer customer,
        ...
        Instant endsAt = period.addTo(startsAt);
        ...
        assertMembershipCorrectness(membership);
    ...
    public Membership continueMembership(Customer customer,
        ...
        Optional<Membership> last = membershipRepository
                .findTopByCustomerOrderByEndsAtDesc(customer);

        if (last.isPresent()) {
            start = last.get().getEndsAt();
        } else {
            start = Instant.now();
        }
    ...
    @Override
    public Membership rescheduleMembership(Long membershipId, Instant newStartsAt) {
        Membership membership = findById(membershipId);

        if (!membership.isActive()) {
            throw new BadRequestException("Inactive memberships cannot be rescheduled");
        }

        Instant newEndsAt = membership.getDuration().addTo(newStartsAt);
        ...
        if (overlap) {
            throw new BadRequestException("Rescheduled membership overlaps with another membership");
        }

        membership.reschedule(newStartsAt, newEndsAt);
    ...
    private void assertMembershipCorrectness(Membership membership) {
        if (membership.getDuration() == null) {
        ...
        if (membershipRepository.existsByCustomerAndActiveTrueAndStartsAtLessThanAndEndsAtGreaterThan(
            ...
            throw new BadRequestException("Membership period overlaps with an existing membership");
        }
